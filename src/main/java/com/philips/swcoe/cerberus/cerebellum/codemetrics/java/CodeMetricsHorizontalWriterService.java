...
public class CodeMetricsHorizontalWriterService extends CodeMetricsWriterService {
    ...
    public CodeMetricsHorizontalWriterService(List<String> classConfig, List<String> methodConfig,
    ...
    private void writeMethodMetrics(CSVPrinter csvPrinter,
        ...
        codeMetricsMethodResults.stream().forEach(codeMetricsMethodResult -> {
            ...
            getStreamMetricsMethods(codeMetricsMethodResult.getClass().getMethods())
                .forEach(methodInMethodResult -> {
                    Try.run(() -> processMetricResult(dataToWriteForMethod,
                        (CodeMetricsDiffResult) methodInMethodResult
                            .invoke(codeMetricsMethodResult), methodConfig))
    ...
    private void writeClassMetrics(CSVPrinter csvPrinter,
        ...
        getStreamMetricsMethods(codeMetricsClassResult.getClass().getMethods())
            .forEach(methodInClassResult -> {
                Try.run(() -> processMetricResult(dataToWriteForClass,
                    (CodeMetricsDiffResult) methodInClassResult.invoke(codeMetricsClassResult),
                    classConfig))
                    .onFailure(exception -> log.trace(exception.getStackTrace()));
            });
    ...
    private void processMetricResult(List<String> dataToWriteForClass,
                                     CodeMetricsDiffResult codeMetricsDiffResult,
                                     List<String> classConfig) {
        Try.of(() -> codeMetricsDiffResult)
            .andThen(codeMetricsResultForClass -> Try.run(
                () -> pushMetricsToWrite(codeMetricsResultForClass, dataToWriteForClass,
                    classConfig))
                .onFailure(exception -> log.trace(exception.getStackTrace())));
    }
    ...
    private void pushMetricsToWrite(CodeMetricsDiffResult codeMetricsDiffResult,
                                    List<String> dataToWrite, List<String> displayConfig) {
